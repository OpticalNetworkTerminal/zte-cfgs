...
PyPI，推送 `v*` tag 发布 TestPyPI。`pyproject.toml` 和
`src/zte_cfgs/__init__.py` 使用带 `v` 的版本字符串，例如 `v0.2.0`；tag 必须
和该版本完全一致。

首次发布前，需要在 PyPI 和 TestPyPI 分别配置 Trusted Publisher：

| 平台 | Environment | Workflow |
| --- | --- | --- |
| PyPI | `pypi` | `.github/workflows/publish.yml` |
| TestPyPI | `testpypi` | `.github/workflows/publish.yml` |
